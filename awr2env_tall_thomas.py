...
def main(dict_args):
    ...
    for fIndx, fnamein in enumerate(dict_args['seabass_file']):
        ...
        if flag[index] != 0:
            ...
            # Append SZA for ThoMaS
            data_out['sza'].append(get_azimuth(lat,lon,ds.dtime[i],0))
    ...
    # add a date field for ThoMaS
    # Add SenZ field for ThoMas
    # date = []
    # time = []
    # senz = []
    # year = data_out['year']
    # for i, yr in enumerate(year):
    #     date.append(f'{yr}{data_out["month"][i]}{data_out["day"][i]}')
    #     time.append(f'{data_out["hour"][i]}:{data_out["minute"][i]}:{data_out["second"][i]}')

    senz = [40] * len(data_out['lat'])

    data_out['SenZ'] = senz
    unit_out['SenZ'] = 'degrees'
    # data_out['time'] = time
    # data_out.move_to_end('time',last='False') # Why won't this work??
    # unit_out['time'] = 'hh:mm:ss'
    # data_out['date'] = date
    # data_out.move_to_end('date',last='False') # Why won't this work??
    # unit_out['date'] = 'yyyymmdd'

    # # Change units for ThoMas
    # unit_out['month'] = 'mm'
    # unit_out['minute'] = 'mm'
    ...
    with open(out_dir / fileout_sb, 'w', encoding="utf-8") as fout:
        ...
        lat_min = min(lat_all)
        lat_max = max(lat_all)
        fout.write(f'/north_latitude={lat_max:0.3f}[DEG]\n')
        fout.write(f'/south_latitude={lat_min:0.3f}[DEG]\n')

        lon_all = [float(loni) for loni in data_out['lon']]
        lon_min = min(lon_all)
        lon_max = max(lon_all)
        fout.write(f'/east_longitude={lon_max:0.3f}[DEG]\n')
        fout.write(f'/west_longitude={lon_min:0.3f}[DEG]\n')

        # For ThoMas
        fout.write('/contact=' + ds.headers['contact'].lower() + '\n')
        #############################################################################################
        #                   THIS IS HARDCODED FOR TURNER SVC DATA                                   #
        # fout.write('/relaz=90\n')                                                                   #
        # fout.write('/raa=90\n')                                                                   #
        fout.write('/RelAz=90\n')                                                                   #
        #############################################################################################
        ...
        #output data matrix
        for i in range(len(data_out['lat'])):
            row_ls = []
